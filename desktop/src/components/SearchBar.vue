...
<template>
  <div class="relative">
    <!-- Search Icon -->
    <svg 
      class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--color-x-text-muted)] transition-colors duration-200 pointer-events-none"
      fill="none" 
      viewBox="0 0 24 24" 
      stroke="currentColor"
      stroke-width="2"
    ...
    <button
      v-if="searchQuery"
      @click="clearSearch"
      class="absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-lg hover:bg-white/20 text-[var(--color-x-text-muted)] hover:text-[var(--color-x-text-primary)] transition-all"
      title="Clear search"
    >
      <svg 
        class="w-4 h-4" 
