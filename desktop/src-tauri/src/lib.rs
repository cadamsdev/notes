...
fn get_db_filename() -> String {
    ...
    println!(
        "TAURI_ENV_USE_TEST_DB: {:?}",
        env::var("TAURI_ENV_USE_TEST_DB")
    );
...
fn create_database_backup(db_path: &PathBuf) -> Result<String, String> {
    ...
    let parent = db_path.parent().ok_or("Failed to get parent directory")?;
    let filename = db_path
        .file_stem()
        .ok_or("Failed to get filename")?
        .to_string_lossy();
    let extension = db_path.extension().unwrap_or_default().to_string_lossy();
...
fn import_database_cmd(
    ...
    fs::copy(&source, &current_db_path).map_err(|e| format!("Failed to import database: {}", e))?;

    println!(
        "Database imported from {} to {}",
        source.display(),
...
fn set_database_directory_cmd(
    ...
    if new_db_path.exists() && new_db_path != old_db_path {
        if backup {
            ...
            println!(
                "Creating backup of existing database at: {}",
                new_db_path.display()
            );
